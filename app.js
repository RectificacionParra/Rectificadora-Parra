...
function generatePDF(q) {
  if (!window.jspdf) { 
    alert("Error: No se pudo cargar el generador de PDF. Revisa tu conexión."); 
    return; 
  }
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();
  ...
  doc.setFont("Helvetica", "bold");
  doc.setFontSize(20);
  doc.text("RECTIFICACIÓN PARRA", 15, 20);
  doc.setFontSize(10);
  doc.setFont("Helvetica", "normal");
  doc.text("Motores y Tapas de Cilindros | Calidad y Precisión", 15, 28);
  doc.setFontSize(12);
  doc.text("Presupuesto: " + q.number, 150, 20);
  ...
  doc.setFont("Helvetica", "normal");
  doc.text("Nombre: " + (client ? client.name : "N/A"), 15, 65);
  doc.text("Teléfono: " + (client ? client.phone : "N/A"), 15, 72);
  doc.text("Trabajo a realizar: " + q.description, 15, 79);
  let y = 100;
  doc.setFont("Helvetica", "bold");
  ...
  items.forEach(function(item) {
    ...
    doc.text(money(item.amount), 170, y, { align: "right" });
    y += 8;
  });
  y += 10;
  doc.line(15, y, 195, y);
  ...
  doc.setTextColor(20, 89, 196);
  doc.text(money(q.total), 195, y, { align: "right" });
  doc.setTextColor(150, 150, 150);
  doc.setFontSize(8);
  doc.text("Gracias por confiar en Rectificación Parra.", 105, 280, { align: "center" });
  doc.save("Presupuesto_" + q.number + ".pdf");
}
const STORAGE_KEY = "recticontrol_v3_pro";
const SESSION_KEY = "recticontrol_session_v1";
const STATES = ["Ingresado", "En proceso", "Terminado", "Cancelado", "Entregado"];
...
function sendWhatsAppQuote(q) {
  ...
  if (!client || !client.phone) {
    alert("El cliente no tiene un teléfono cargado.");
    return;
  }
